...
#     cmd1 = ["samtools view -h %s | tee >(grep '^@\|watson\|Watson' |"%(in_files['sam_out']['merged'])+
#             "samtools view -Shb - |samtools sort -o %s )|"%
#             (watson_merged.name)+
#             "grep '^@\|crick\|Crick' |samtools view -Shb - |samtools sort -o %s"%
#             (crick_merged.name)]
#     cmd2 = ["samtools view -h %s | tee >(grep '^@\|watson\|Watson' |"%(in_files['sam_out']['joined'])+
#             "samtools view -Shb - |samtools sort -o %s)|"%
#             (watson_joined.name)+
#             "grep '^@\|crick\|Crick' |samtools view -Shb - |samtools sort -o %s"%
#             (crick_joined.name)]
...
#         cmd1 = ['samtools-0.1.18 mpileup -u %s | bcftools_old view -cg - | %s vcf2fq |%s seq -A - > %s'%
#                 (in_files['sam_out']['crick_%s'%type],vcfutils,seqtk,crick_out.name)]
#         cmd2 = ['samtools-0.1.18 mpileup -u %s | bcftools_old view -cg - | %s vcf2fq |%s seq -A -  > %s'%
