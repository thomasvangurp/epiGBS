...
def parse_vcf(args):
    ...
    watson_file = open(args.watson, 'r')
    crick_file = open(args.crick, 'r')
    reference_genome = SeqIO.to_dict(SeqIO.parse(args.reference, "fasta"))  # reference genome .fasta file
    methylation_file = open(args.methylation_output, 'w')
    snp_file = open(args.SNP_output, 'w')
    ...
    while True:
        ...
        if len([s for s in crick_record.samples if s.data.DP not in [0, None]]) < 0.5 * len(watson_record.samples):
            continue
        if int(watson_record.CHROM) < 14050:
            continue
        print watson_record.CHROM
